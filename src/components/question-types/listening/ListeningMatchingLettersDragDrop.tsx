...
interface ListeningMatchingLettersDragDropProps {
  ...
  instructions?: string
  statements: string[]      // List of questions/statements to be matched
...
const ListeningMatchingLettersDragDrop: React.FC<ListeningMatchingLettersDragDropProps> = ({
  questionRange,
  instructions,
  statements = [],
  letters = [],
  ...
  // Check if we have more questions than available letters
  const hasMoreQuestionsThanLetters = statements.length > letters.length
  
  // If we have more questions than letters, keep all letters available
      ...
      <div className="grid grid-cols-1 md:grid-cols-[260px_1fr] gap-4 items-start">
        ...
        <div className="space-y-3 text-[13px]">
          <div className="flex items-center justify-between">
            ...
            {displayRange ? (
              <span className="text-[11px] text-gray-600 bg-gray-100 px-1.5 py-0.5 rounded">{displayRange}</span>
            ) : null}
          </div>
          {instructions ? (
            <div className="bg-blue-50 border border-blue-200 rounded-md p-2 text-[12px] text-blue-800">
              <RichTextViewer content={instructions} enableSelectionHighlight />
            </div>
          ) : null}
              ...
              <div key={index} className="border border-gray-200 rounded-md p-2.5 bg-white">
                <div className="flex gap-3 items-start">
                  ...
                  <div className="flex-1 leading-snug">
                    <RichTextViewer content={statement} enableSelectionHighlight />
                  </div>
...
export default ListeningMatchingLettersDragDrop
